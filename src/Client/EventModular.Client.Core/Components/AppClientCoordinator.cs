...
using EventModular.Shared.Controllers.Identity;
using EventModular.Client.Core.Services.DiagnosticLog;

namespace EventModular.Client.Core.Components;
    ...
    private void SubscribeToSignalREventsMessages()
    {
        signalROnDisposables.Add(hubConnection.On<string, object, bool>(SignalREvents.SHOW_MESSAGE, async (message, data) =>
        {
            ...
            if (await notification.IsNotificationAvailable())
            {
                ...
                await notification.Show("EventModular SignalR", new()
                {
            ...
            else
            {
                ...
                SnackBarService.Show("EventModular", message);
            }
